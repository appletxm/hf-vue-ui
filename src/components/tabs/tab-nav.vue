...
export default {
  ...
  render() {
    ...
    const scrollBtn = scrollable
      ? [
        <span class={[`${this.cfg.prefix}-tabs__nav-prev`, scrollable.prev ? '' : 'is-disabled']} on-click={scrollPrev}><i class={`${this.cfg.prefix}-icon ui-icon-line-direction-left`}></i></span>,
        <span class={[`${this.cfg.prefix}-tabs__nav-next`, scrollable.next ? '' : 'is-disabled']} on-click={scrollNext}><i class={`${this.cfg.prefix}-icon ui-icon-line-direction-right`}></i></span>
      ] : null;
    const tabs = this._l(panes, (pane, index) => {
      ...
      const btnClose = closable
        ? <span class={`${this.cfg.prefix}-tabs__item--icon-close ${this.cfg.prefix}-icon ui-icon-line-currency-close`} on-click={(ev) => { onTabRemove(pane, ev); }}></span>
        : null;

      const tabLabelContent = pane.$slots.label || pane.label;
      const tabindex = pane.active ? 0 : -1;
      const tabContent = type === 'ellipse' ? (<div
          class={{
            [`${this.cfg.prefix}-tabs__item`]: true,
        ...
        >
          <span class={`${this.cfg.prefix}-tabs__item--content`}>
            {tabLabelContent}
            {btnClose}
          </span>
        </div>) : (<div
    ...
    return (
      <div class={[`${this.cfg.prefix}-tabs__nav-wrap`, scrollable ? 'is-scrollable' : '', `is-${this.rootTabs.tabPosition}`]}>
        {scrollBtn}
        <div class={[`${this.cfg.prefix}-tabs__nav-scroll`]} ref="navScroll">
          <div
            class={[`${this.cfg.prefix}-tabs__nav`, `is-${this.rootTabs.tabPosition}`, stretch && ['top', 'bottom'].indexOf(this.rootTabs.tabPosition) !== -1 ? 'is-stretch' : '']}
            ref="nav"
            style={navStyle}
            role="tablist"
            on-keydown={ changeTab }
          >
            {!type || type === 'line' || type === 'ellipse' ? <tab-bar tabs={panes}></tab-bar> : null}
            {tabs}
          </div>
        </div>
      </div>

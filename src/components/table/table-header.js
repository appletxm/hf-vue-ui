...
export default {
  ...
  render(h) {
    ...
    return (
      <table
        class={ [`${this.cfg.prefix}-table__header`] }
        cellspacing="0"
        cellpadding="0"
        border="0"
        >
        <colgroup>
          {
            originColumns.map((column) => <col name={ column.id } key={column.id} />)
          }
          {
                    ...
                    <div class={ ['cell', column.filteredValue && column.filteredValue.length > 0 ? 'highlight' : '', column.labelClassName] }>
                      ...
                      }
                      {
                        column.filterable ? (<span
                          class={ [`${this.cfg.prefix}--table__column-filter-trigger`] }
                          on-click={ ($event) => this.handleFilterClick($event, column) }>
                          <i class={ [`${this.cfg.prefix}-icon-arrow-down`, column.filterOpened ? `${this.cfg.prefix}-icon-arrow-up` : ''] }></i>
                        </span>) : ''
                      }
                    </div>
                  </th>))

...
export default {
  ...
  render() {
    // old const data = this.data || []; 不生效
    const data = this.store.states.data || [];
    const { columns } = this.store.states
    ...
    return (
      <table
        class={ `${this.cfg.prefix}-table__body` }
        cellspacing="0"
        cellpadding="0"
        border="0">
        <colgroup>
        ...
        <tbody>
          ...
          <hf-ui-pop-tip effect={ this.table.tooltipEffect } placement="top" ref="tooltip" content={ this.tooltipContent }></hf-ui-pop-tip>
        </tbody>
      </table>
    );
  },
  ...
  watch: {
    ...
    'store.states.hoverRow'(newVal, oldVal) {
      ...
      raf(() => {
        const rows = this.$el.querySelectorAll(`${this.cfg.prefix}-table__row`);
  ...
  methods: {
    ...
    getRowClass(row, rowIndex) {
      const classes = [`${this.cfg.prefix}-table__row`];
      if (this.table.highlightCurrentRow && row === this.store.states.currentRow) {
        classes.push('current-row');
      }

      if (this.stripe && rowIndex % 2 === 1) {
        classes.push(`${this.cfg.prefix}-table__row--striped`);
      }
    ...
    rowRender(row, $index, treeRowData) {
      ...
      let display = true;
      if (treeRowData) {
        rowClasses.push(`${this.cfg.prefix}-table__row--level-` + treeRowData.level);
      ...
      if (this.hasExpandColumn && isRowExpanded(row)) {
        ...
        return [[
          ...
          <tr key={'expanded-row__' + tr.key}>
            <td colspan={ this.columnsCount } class={ '{this.cgf.prefix}-table__expanded-cell' }>

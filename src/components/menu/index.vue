...
export default {
      ...
      render(createElement, context) {
        const data = {
          ...
          on: {
            ...
            enter(el) {
              addClass(el, `${cfg.prefix}-opacity-transition`);
              el.style.opacity = 1;
            },

            afterEnter(el) {
              removeClass(el, `${cfg.prefix}-opacity-transition`);
              el.style.opacity = '';
            },
            ...
            beforeLeave(el) {
              ...
              if (hasClass(el, `${cfg.prefix}-menu--collapse`)) {
                removeClass(el, `${cfg.prefix}-menu--collapse`);
                el.dataset.oldOverflow = el.style.overflow;
                el.dataset.scrollWidth = el.clientWidth;
                addClass(el, `${cfg.prefix}-menu--collapse`);
              } else {
                addClass(el, `${cfg.prefix}-menu--collapse`);
                el.dataset.oldOverflow = el.style.overflow;
                el.dataset.scrollWidth = el.clientWidth;
  ...
  computed: {
    ...
    isMenuPopup() {
      return this.mode === 'horizontal' || (this.mode === 'vertical' && this.collapse) || (this.mode === 'vertical' && this.type === 'popup');
    }
  },
  watch: {
    defaultActive(value) {
  ...
  methods: {
    ...
    openMenu(index, indexPath) {
      if (this.type === 'popup') {
        const oldIndexArr = this.activeIndex.split('-');
        const currentIndexArr = index.split('-');
    ...
    handleItemClick(item) {
      ...
      if (this.mode === 'horizontal' || this.collapse || this.type === 'popup') {
        this.openedMenus = [];
      }
    ...
    initOpenedMenu() {
      ...
      if (!activeItem || this.mode === 'horizontal' || this.collapse || this.type === 'popup') return;
  ...
  render() {
    const component = (
      <ul
        ...
        class={[
          `${cfg.prefix}-menu`,
          this.mode === 'horizontal' ? `${cfg.prefix}-menu--horizontal` : `${cfg.prefix}-menu--vertical`,
          this.collapse ? `${cfg.prefix}-menu--collapse` : ''
        ].join(' ')}
      >
        { this.$slots.default }
      </ul>
